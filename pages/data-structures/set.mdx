...
7. forEach(callbackfn) - an iteration method that executes the provided function once for each value in the set.

JavaScript ES6 (ES2015) has a built-in Set object, which contains all of the methods above (set.size in ES6 is a number property and not a method).
...
Using the Set class above, we can create a new set using:
...
const set = new SetExt<Person>([{ name: "Karla", age: 26 }]);
```
