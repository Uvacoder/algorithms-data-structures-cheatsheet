...
A common programming exercise that is solved with a hash table is counting the frequency of words, given a phrase or paragraph. The key is the word and the value is the frequency.
...
A collision happens when the hash function assigns two different key inputs to the same hash number. Hash functions should be well-written to avoid collision.
...
## Simple Hash Table Implementation in TypeScript

Not needed irl because JavaScript has a Map object, but it's useful to see how the methods are implemented at a basic level to understand under the hood.
...
class HashTable<T> implements IHashTable<T> {
    ...
    constructor(initialItems: HashTablePair<T>[], size: number) {
    ...
    private getHash(key: string, tableSize: number): number {
        ...
        return hash % tableSize;  // use modulo operator to solve the problem of the hash getting too big
    }

    has(key: string): boolean {
        const index = this.getHash(key, this.table.length);
        if (!!this.table[index]) {
            if (this.table[index].find((pair: any) => pair[0] === key)) {
                return true;
            }
        }
        return false;
    ...
    get(key: string): T | undefined {
        ...
        if (this.table[index].length > 0) {
            ...
            const pair = this.table[index].find((pair: any) => pair[0] === key);
            if (pair) {
                return pair[1];
            }
        }
...
Maybe when I have the time, I'll include that in the implementation as well. But if you're feeling like a tinkerer, feel free to play around and implement it yourself ;)
